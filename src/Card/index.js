...
const Card = ({animal,matchedCards, idCardToFlipDown, hasFirstFlip, flipFirstCard,flipSecondCard}) =>{
    ...
    useEffect( () => { 
          if (flip) { //determines which card is flipped
            if (hasFirstFlip) {
              flipSecondCard(animal);
            } else {
              flipFirstCard(animal);
            }
          }else{ //if same card is clicked twice - reset property
            flipFirstCard("");
          }
        },[flip]);
    ...
    useEffect(()=>{
        ...
        if(matchedCards.includes(animal)){  //if card has a match - disable from being clicked
            setDisableFlip(true);
        }
    ...
    return(
        ...
        <div className={`card ${flip ? 'flip' : ''}`} id={animal} onClick={!disableFlip? flipCard : undefined}>
